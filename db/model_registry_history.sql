...
-- 添加表备注
COMMENT ON TABLE model_registry_history IS '模型注册表历史表，存储模型每次变动的记录。';

-- 添加列备注
COMMENT ON COLUMN model_registry_history.model_id IS '对应 model_registry.id 外键。';
COMMENT ON COLUMN model_registry_history.model_name IS '模型名称。';
COMMENT ON COLUMN model_registry_history.model_type IS '模型类型。';
...
COMMENT ON COLUMN model_registry_history.status IS '模型的生效状态：active|inactive|archived|pending。';
COMMENT ON COLUMN model_registry_history.change_type IS '变更类型：create, update, activate, deactivate';
COMMENT ON COLUMN model_registry_history.changed_at IS '记录变更时间。';
COMMENT ON COLUMN model_registry_history.changed_by IS '记录变更人员。';
COMMENT ON COLUMN model_registry_history.remarks IS '更新内容备注。';
...
CREATE TRIGGER trg_update_model_registry_history
BEFORE UPDATE ON model_registry_history
FOR EACH ROW
