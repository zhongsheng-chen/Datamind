...
-- 添加表备注
COMMENT ON TABLE registry IS '模型注册表，存储模型的元数据信息。';

-- 添加列备注
COMMENT ON COLUMN registry.id IS '自增主键。';
COMMENT ON COLUMN registry.model_name IS '模型名称。';
COMMENT ON COLUMN registry.model_type IS '模型类型：decision_tree|random_forest|xgboost|lightgbm|logistic_regression。';
...
COMMENT ON COLUMN registry.updated_by IS '更新模型的人员。';

-- 创建索引,提高查询的效率
CREATE UNIQUE INDEX IF NOT EXISTS uq_idx_registry_uuid ON registry (uuid);
CREATE INDEX IF NOT EXISTS idx_registry_hash ON registry (hash);
...
CREATE INDEX IF NOT EXISTS idx_registry_task ON registry (task);
CREATE INDEX IF NOT EXISTS idx_registry_name_version ON registry (model_name, version);
CREATE INDEX IF NOT EXISTS idx_registry_name_version_status ON registry (model_name, version, status);
