# datamind/core/scoring/adapters/catboost.py

"""CatBoost 模型适配器

支持 CatBoost 原生模型的适配器实现。
...
特性：
  - 原生支持：直接使用 CatBoost 的 predict_proba 方法
  - 特征重要性提取：从模型中提取 PredictionValuesChange 类型的重要性
...
class CatBoostAdapter(BaseModelAdapter):
    ...
    def _extract_feature_names_from_model(self):
        ...
        try:
            if hasattr(self.model, 'feature_names_'):
                ...
                _logger.debug("成功提取 CatBoost 特征名，数量: %d", len(self.feature_names))
            else:
                _logger.debug("CatBoost 模型无 feature_names_ 属性，将使用默认命名")
        except Exception as e:
            _logger.debug("提取 CatBoost 特征名失败: %s", e)
    ...
    def predict_proba(self, X: np.ndarray) -> float:
        ...
        try:
            ...
            _logger.error("CatBoost 单条预测失败: %s", e)
            raise
    ...
    def predict_proba_batch(self, X: np.ndarray) -> List[float]:
        ...
        try:
            ...
            _logger.error("CatBoost 批量预测失败: %s", e)
            raise
    ...
    def decision_function(self, X: np.ndarray) -> float:
        ...
        返回:
            logit 值
        """
        # CatBoost 使用 prediction_type='RawFormulaVal' 获取原始分数
        return float(self.model.predict(X, prediction_type='RawFormulaVal')[0])
    ...
    def decision_function_batch(self, X: np.ndarray) -> List[float]:
        ...
        返回:
            logit 值列表
        """
        return self.model.predict(X, prediction_type='RawFormulaVal').tolist()
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            if hasattr(self.model, 'get_feature_importance'):
                values = self.model.get_feature_importance(type='PredictionValuesChange')
            ...
            _logger.error("获取 CatBoost 特征重要性失败: %s", e)
