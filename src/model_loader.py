...
class ModelLoader:
    ...
    def load_model(cls, model_name: str) -> CachedModel:
        ...
        with cls._lock:
            if model_name not in cls._model_cache:
                logger.debug(f"[缓存未命中] 模型 {model_name}，正在加载...")
    ...
    def reload_model(cls, model_name: str) -> CachedModel:
        ...
        with cls._lock:
            ...
            if model_name in cls._model_cache:
                logger.warning(f"[重新加载] 加载模型 {model_name}")
    ...
    def load_all_models(cls):
        ...
        if cls._all_models_loaded:
            logger.info("[完成] 所有模型已加载，无需重复加载")
            return
        ...
        cls._all_models_loaded = True

        elapsed = time.time() - start_time
        logger.info(f"成功 {len(success_models)} 个，失败 {len(failed_models)} 个，总耗时 {elapsed:.2f} 秒")
        if failed_models:
